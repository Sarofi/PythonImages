...
def inst_Neta2_rule49_Nv500_homo_inv():
    ...
    cmd_dict['imagestackformat']='mrc'
    cmd_dict['fn_imagestack']='FHV.out.lmax10pmax5.Neta2.rule49.Nv500.fw.imagestack.mrc'
    cmd_dict['startSlice']=1
    cmd_dict['numSlices']=500
    cmd.append(cmd_dict)
    ...
    ZhyeYinresolutionsteps['Nic']=np.array([10,2,2]) # This is for inv.RUN1.m
    ...
    MultiplierForPixelnoisevarIC_eachstep=np.array([10.0,4.0,1.0])
    #####################
    cmd.append({'operator':'misc_diary','fn_diary':'off'})
    #####################
    ...
    for step in range(ZhyeYinresolutionsteps['lmax'].size):
        #####################
        cmd.append({'operator':'misc_changedirectory', 'dn':'step'+str(step)})
        #####################
        cmd.append({'operator':'misc_diary','fn_diary':outputbasename+'.diary.txt'})
        #####################
        cmd_dict = {}
        ...
        cmd.append({'operator':'EM_set_2Dreciprocal_in_virusobj','use_vkminimalset_rather_than_vk':False})
        #####################
        cmd.append({'operator':'quad_read_integration_rule','fn_rule':'/Users/YunHan/Google Drive/WorkSpace/FHV2014/rule_small_3_rulechopper'})
        #####################
        cmd_dict = {}
        ...
        cmd_dict['maxiter4pixelnoisevarupdate']=5
